...
gird = [list(map(int, line.strip())) for line in open('input.txt').readlines()]
...
while q:
    ...
    if row == len(gird) - 1 and col == len(gird[0]) - 1 and steps >= 4:
        print(heat_loss)
        break
    ...
    if steps < 10 and (direction_row, direction_col) != (0, 0):
    ...
    if steps >= 4 or (direction_row, direction_col) == (0, 0):
